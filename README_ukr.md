...
> Це посилання працює лише для пилососів `dreame.vacuum`. Модель можна знайти в [специфікації](https://home.miot-spec.com/). Так легко можна побачити та завантажити усі офіційно доступні голосові пакети.
...
Голосовий пакет Xiaomi — це архів `tar.gz`, який містить `.mp3` файли без папок (усі файли розміщені просто в корені архіву). Усі файли названі цифрами. Кожна така цифра відповідає конкретній фразі. Детальніше дивись [мапінг команд Xiaomi](https://github.com/sverdlyuk/glados_ukr/blob/main/xiaomi_glados_uk_mapping.csv). Бітрейт - 96 кбіт/с. Частота дискретизації — 44,1 кГц або 16,0 кГц.
...
## 🎙️ Як створити власний голосовий пакет Ijai
Голосовий пакет Ijai — це архів `.zip`, який містить `.mp3`, 32-bit float, 44100 Гц. Файли всередині архіву без папок (усі файли розміщені просто в корені архіву). Усі файли мають чітко визначені імена накшталт `3sound_appoint_area_clean_start.mp3`. Детальніше дивись мапінг команд Ijai. Також разом з файлами в архіві має бути файл `md5.ini` Це список назв файлів разом з MD5 сумами кожного. Для зручності цей файл можна згенерувати автоматично використавши таку команду в Linux, або MacOS:
```
